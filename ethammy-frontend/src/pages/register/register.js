...
class RegisterForm extends Component {
  ...
  onConfirmPasswordChange = (e) => {
    this.onChange(e);
    this.setState({
      confirmPasswordError: Validate.matchPasswords(
        this.state.password,
        e.target.value
      ),
    });
  ...
  render() {
    ...
    let nameErrorP,
      ...
      registerErrorP,
      button;
    if (nameError === constant.ERROR)
      nameErrorP = <p>{constant.nameErrorMsg}</p>;
    if (emailError === constant.ERROR)
      emailErrorP = <p>{constant.emailErrorMsg}</p>;
    if (usernameError === constant.ERROR)
      usernameErrorP = <p>{constant.usernameErrorMsg}</p>;
    if (passwordError === constant.ERROR)
      passwordErrorP = <p>{constant.passErrorMsg}</p>;
    if (confirmPasswordError === constant.ERROR)
      confirmPasswordErrorP = <p>{constant.passMatchErrorMsg}</p>;
    if (registerError) registerErrorP = <p>{constant.registerErrorMsg}</p>;

    const clickable =
      nameError === constant.ACCEPTED &&
      emailError === constant.ACCEPTED &&
      usernameError === constant.ACCEPTED &&
      passwordError === constant.ACCEPTED &&
      confirmPasswordError === constant.ACCEPTED;
    if (clickable)
      button = (
